 ...
 *
 */
public class Topology2DTriangularMesh extends Topology2D {
	...
	public void defineTopology() {
		 ...
		 */
		int[] tmp_edgeLeftNeighbour = new int[3*elementsVertices.size()];
		int[] tmp_edgeRightNeighbour = new int[3*elementsVertices.size()];
		int[] loc_ind_S = new int[3*elementsVertices.size()];
		int[][] tmp_elementEdgesSet = new int[3*elementsVertices.size()][3];
		int[][] tmp_edgesSet = new int[3*elementsVertices.size()][2];

		int N_ins = 0;
		for(int element=1; element<elementsVertices.size(); element++) { 

			loc_ind_S[element] = 0;
			int j1 = elementInVector(sort2(elementsVertices.get(element)[0],elementsVertices.get(element)[1]),tmp_edgesSet,N_ins, 3*elementsVertices.size());
			int j2 = elementInVector(sort2(elementsVertices.get(element)[1],elementsVertices.get(element)[2]),tmp_edgesSet,N_ins, 3*elementsVertices.size());
			int j3 = elementInVector(sort2(elementsVertices.get(element)[2],elementsVertices.get(element)[0]),tmp_edgesSet,N_ins, 3*elementsVertices.size());
			...
			if(j3==0) {
				N_ins = N_ins + 1;
				loc_ind_S[element] = loc_ind_S[element] +1;
				tmp_edgesSet[N_ins][0] = sort2(elementsVertices.get(element)[2],elementsVertices.get(element)[0])[0];
				tmp_edgesSet[N_ins][1] = sort2(elementsVertices.get(element)[2],elementsVertices.get(element)[0])[1];
				tmp_elementEdgesSet[element][loc_ind_S[element]] = N_ins;
				tmp_edgeLeftNeighbour[N_ins] = element;		
			} else {
				loc_ind_S[element] = loc_ind_S[element] +1;
		...
		for(int element=1; element<elementsVertices.size(); element++) {
			...
			for(int i=0; i<3; i++) {
				...
				if(tmp_edgeLeftNeighbour[edge] == element) {
					...
					} else if(tmp_edgesSet[edge][0] == elementsVertices.get(element)[0] & tmp_edgesSet[edge][1] == elementsVertices.get(element)[2] ||
							tmp_edgesSet[edge][1] == elementsVertices.get(element)[0] & tmp_edgesSet[edge][0] == elementsVertices.get(element)[2]) {
		...
		edgesSet.add(0, new Integer[] {-9999,-9999,-9999});
		...
		for(int i=1; i<elementsVertices.size(); i++) {
			...
			elementEdgesSet.add( i, new ArrayList<Integer>(Arrays.asList(tmp_elementEdgesSet[i][0],tmp_elementEdgesSet[i][1],tmp_elementEdgesSet[i][2])) );
			...
			elementNeighbours.add( i, new ArrayList<Integer>(Arrays.asList(tmp_p[0],tmp_p[1],tmp_p[2]) ) );
		}
		...
		if(checkData == true) {
			...
			for(int element=1; element<elementEdgesSet.size(); element++) {
				...
				System.out.println( "\t\telement " + element + " : " + elementEdgesSet.get(element).get(0) + "," + elementEdgesSet.get(element).get(1) + "," + elementEdgesSet.get(element).get(2) ); 
